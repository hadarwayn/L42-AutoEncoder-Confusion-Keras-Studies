"""Experiment A — Human Faces: Train on males, confuse with female faces."""
from typing import Dict, Any

from src.autoencoder import build_autoencoder, DEFAULT_CONFIG_FACES
...
from src.utils.data_loader import load_human_faces, verify_data
from src.utils.paths import get_results_dir
from src.utils.logger import setup_logger

logger = setup_logger("experiment_a")

EXPERIMENT = "experiment_a"
NAME = "Human Faces (Male-Trained vs Female-Tested)"


def run(epochs: int = 30, batch_size: int = None) -> Dict[str, Any]:
    """Run the full Experiment A pipeline: load, train, confuse, visualize."""
    ...
    # 1. Load data
    logger.info("=" * 60)
    logger.info("EXPERIMENT A — Human Faces: Males vs Females")
    logger.info("=" * 60)
    x_train, x_val, x_test = load_human_faces()
    verify_data(x_train, x_val, x_test, NAME)

    # 2. Build model
    models = build_autoencoder(DEFAULT_CONFIG_FACES)
    ...
    logger.info(f"Experiment A complete — all results in {results_dir}")
